...
export class DataService<Type> {
    ...
    public queryData(ns: string, owner: any): Observable<Type[]> {
        console.log('searchAll ' + this.actionUrl + ns + '?owner=' + owner);
        return this.http.get(this.actionUrl + ns + '?owner=resource%3Aorg.acme.mynetwork.Trader%23' + owner)
            .map(this.extractData)
            .catch(this.handleError);
    }

    public searchTrader(ns: string): Observable<Type[]> {
        console.log('searchTrader ' + this.actionUrl + ns);
        return this.http.get(this.actionUrl + ns)
            .map(this.extractData)
            .catch(this.handleError);
    }

    public addTrader(ns: string, data: Type): Observable<Type> {
